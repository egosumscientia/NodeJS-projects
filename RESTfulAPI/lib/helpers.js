...
helpers.sendTwilioSms = function(phone,msg,callback){
    ...
    if(phone && msg){
        //Configure the request payload which is going to be sent to Twilio
        let payload = {
        ...
        //Configuring the request's details
        var requestDetails = {
            'protocol' : 'https:',
        ...
        //Instantiating a request object
    ...
    helpers.getTemplate('_header',data,function(err,headerString){
        if(!err && headerString){
            ...
            helpers.getTemplate('_footer',data,function(err,footerString){
                if(!err && footerString){
                    //Add the three (3) strings together
...
helpers.interpolate = function(str,data){
    str = typeof(str) == 'string' && str.length > 0 ? str : '';
    data = typeof(data) == 'object' && data !== null ? data : {};
    //Add the templateGlobals to the data object, prepending their name with "global"
    for(let keyName in config.templateGlobals){
        if(config.templateGlobals.hasOwnProperty(keyName)){
            data['global.'+keyName] = config.templateGlobals[keyName];
        }
    };
    //For eeach key in the data object, insert its value into the string at the correspondng placeholder
    for(let key in data){
